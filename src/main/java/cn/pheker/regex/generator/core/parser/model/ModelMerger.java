...
public class ModelMerger {
     ...
     * @return 合并是否成功
     */
    public Branches merge(Sequence other) {
        // 空节点处理
    ...
    private void doMerge(Node first, Node second) {
        ...
        if (first.isExtendsOf(Pair.class)) {
            ...
            if (!firstPair.getStart().equals(secondPair)) {
                mergeAsBranches(first, second);
                // 相同类型的Pair
            } else {
    ...
    private void mergePair(Pair firstPair, Pair secondPair) {
        ...
        // 2.逐个合并相同类型前缀节点
        // 3.一次性合并不同类型后缀节点
        int minLen = Math.min(firstPair.size(), secondPair.size());
        for (int i = 1; i < minLen; i++) {
            ...
            if (isStructSame(firstSub, secondSub)) {
                ...
                List<Node> nodes1 = removeFrom(firstPair.children(), i, firstPair.size() - 1);
                Sequence firstBranch = newSequence(nodes1);
                List<Node> nodes2 = removeFrom(secondPair.children(), i, secondPair.size() - 1);
                Sequence secondBranch = newSequence(nodes2);
